...
int main ()
{
  ...
  **/
  ...
  double tau_p_throttle = 0.125;  
  double tau_d_throttle = 0.05;  
  double tau_i_throttle = 0.0125;  
  ...
  double tau_p_steer = 0.3;  
  double tau_d_steer = 0.0;  
  double tau_i_steer = 0.0;  
        ...
        if (s != "") {
          ...
          **/
		  ...
		  if (dot_arg > 0.0) {
			  ...
			  yaw_main = angle_between_points(x_position, y_position, x_avg, y_avg);              
...
//           // Update the delta time with the previous command
          ...
          **/		  	  		  
		  ...
		  double vel_comp = 0.0;
		  double vel_comp_max = 1.0; //Maximum ammount of velocity compensation
		  if (dot_arg > 0.0) {
